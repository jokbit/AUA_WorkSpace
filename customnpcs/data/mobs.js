var mobs = [
  "minecraft:zombie",
  "minecraft:zombie_villager",
  "minecraft:skeleton",
  "minecraft:stray",
  "minecraft:husk",
  "minecraft:drowned",
  "minecraft:ghast",
  "minecraft:creeper",
  "minecraft:giant",
  "apocalypse:destroyer",
  "apocalypse:seeker",
  "mutantszombies:blister_zombie",
  "mutantszombies:crawler",
  "mutantszombies:zombie_brute",
  "mutantszombies:spitter",
  "mutantszombies:split_head_zombie",
  "mutantszombies:mutant_brute",
  "mutantszombies:rotten_mutant",
  "angry_animals:angry_pig",
  "angry_animals:angry_cow",
  "angry_animals:angry_sheep",
  "angry_animals:angry_chicken",
  "angry_animals:angry_wolf",
  "angry_animals:angry_ocelot",
  "angry_animals:angry_bee",
  "angry_animals:angry_fox",
  "angry_animals:angry_goat",
  "angry_animals:angry_llama",
  "angry_animals:shadow_angry_cow",
  "angry_animals:angry_golden_pig",
  "much_more_skeletons:skeleton_pig",
  "much_more_skeletons:skeleton_cow",
  "much_more_skeletons:skeleton_chicken",
  "much_more_skeletons:skeleton_wolf",
  "much_more_skeletons:skeleton_cat",
  "much_more_skeletons:skeleton_villager",
  "much_more_skeletons:skeleton_enderman",
  "much_more_skeletons:dark_skeleton",
  "much_more_skeletons:ender_skeleton",
  "much_more_skeletons:husk_skeleton",
  "much_more_skeletons:girl_skeleton",
  "much_more_skeletons:skeleton_ravager",
  "much_more_skeletons:skeleton_spider",
  "much_more_skeletons:zombie_skeleton",
  "much_more_skeletons:undead_skeleton",
  "much_more_skeletons:wither_stray",
  "much_more_skeletons:nether_skeleton",
  "much_more_skeletons:skeleton_piglin",
  "much_more_skeletons:hell_skeleton",
  "much_more_skeletons:jungle_skeleton",
  "much_more_skeletons:swamp_skeleton",
  "much_more_skeletons:soul_sand_skeleton",
  "much_more_skeletons:soul_fire_skeleton",
  "much_more_skeletons:lava_skeleton",
  "much_more_skeletons:tainted_skeleton",
  "much_more_skeletons:warped_skeleton",
  "much_more_skeletons:crimson_skeleton",
  "much_more_skeletons:infected_skeleton",
  "much_more_skeletons:wither_skeleton_enderman",
  "much_more_skeletons:wither_skeleton_wolf",
  "much_more_skeletons:wither_skeleton_cat",
  "much_more_skeletons:wither_skeleton_villager",
  "much_more_skeletons:skeleton_fox",
  "much_more_skeletons:wither_skeleton_fox",
  "much_more_skeletons:skeleton_hoglin",
  "much_more_skeletons:wither_skeleton_hoglin",
  "much_more_skeletons:skeleton_ghast",
  "mobz:tank_zombie",
  "mobz:fast_zombie",
  "mobz:armored_zombie",
  "mobz:ender_zombie",
  "mobz:boss_skeleton",
  "mobz:overgrown_skeleton",
  "mobz:nether_skeleton",
  "mobz:small_zombie",
  "mobz:bigboss",
  "born_in_chaos_v1:decrepit_skeleton",
  "born_in_chaos_v1:skeleton_demoman",
  "born_in_chaos_v1:baby_skeleton",
  "born_in_chaos_v1:baby_skeleton_minion",
  "born_in_chaos_v1:bone_imp",
  "born_in_chaos_v1:bone_imp_minion",
  "born_in_chaos_v1:siamese_skeletons",
  "born_in_chaos_v1:siamese_skeletonsleft",
  "born_in_chaos_v1:siamese_skeletonsleft",
  "born_in_chaos_v1:skeleton_thrasher",
  "born_in_chaos_v1:bonescaller",
  "born_in_chaos_v1:supreme_bonescaller",
  "born_in_chaos_v1:supreme_bonescaller_stage_2",
  "born_in_chaos_v1:spirit_guide",
  "born_in_chaos_v1:spirit_guide_assistant",
  "born_in_chaos_v1:decaying_zombie",
  "born_in_chaos_v1:barrel_zombie",
  "born_in_chaos_v1:door_knight",
  "born_in_chaos_v1:zombie_clown",
  "born_in_chaos_v1:zombie_fisherman",
  "born_in_chaos_v1:zombie_lumberjack",
  "born_in_chaos_v1:zombie_bruiser",
  "born_in_chaos_v1:maggot",
  "born_in_chaos_v1:restless_spirit",
  "born_in_chaos_v1:scarlet_persecutor",
  "born_in_chaos_v1:pumpkin_spirit",
  "born_in_chaos_v1:seared_spirit",
  "born_in_chaos_v1:mr_pumpkin_controlled",
  "born_in_chaos_v1:controlled_spiritual_assistant",
  "born_in_chaos_v1:controlled_baby_skeleton",
  "born_in_chaos_v1:corpse_fish",
  "infectious:angry_zombie",
  "infectious:infectious_zombie",
  "infectious:fat_zombie",
  "infectious:old_zombie",
  "infectious:zombie_girl",
  "infectious:screamer",
  "infectious:chained_zombie",
  "infectious:balloon_zombie",
  "infectious:sleepy_zombie",
  "infectious:zombie_spiter",
  "infectious:tower_zombie",
  "infectious:zombie_on_stelts",
  "infectious:big_zombie",
  "infectious:zombie_head",
  "infectious:brain_zombie",
  "infectious:blind_zombie",
  "infectious:crawling_zombie",
  "infectious:armless_zombie",
  "infectious:bodyless_zombie",
  "infectious:headless_zombie",
  "infectious:maggot_zombie",
  "infectious:claw_zombie",
  "infectious:bucket_zombie",
  "infectious:pylon_zombie",
  "infectious:pumpkin_zombie",
  "infectious:beehive_zombie",
  "infectious:barrel_zombie",
  "infectious:door_zombie",
  "infectious:iron_door_zombie",
  "infectious:football_player_zombie",
  "infectious:bouncer_zombie",
  "infectious:zombie_climber",
  "infectious:zombie_runner",
  "infectious:zombie_boxer",
  "infectious:muscular_zombie",
  "infectious:pool_tube_zombie",
  "infectious:zombie_diver",
  "infectious:deep_sea_diver_zombie",
  "infectious:zombie_farmer",
  "infectious:zombie_fisherman",
  "infectious:beekeeper_zombie",
  "infectious:zombie_lumberjack",
  "infectious:zombie_miner",
  "infectious:construction_worker_zombie",
  "infectious:zombie_painter",
  "infectious:zombie_clown",
  "infectious:disco_zombie",
  "infectious:zombie_dancer",
  "infectious:mc_donalds_worker_zombie",
  "infectious:zombie_cook",
  "infectious:zombie_doctor",
  "infectious:zombie_astronaut",
  "infectious:robotic_zombie",
  "infectious:zombie_scientist",
  "infectious:fire_fighter_zombie",
  "infectious:zombie_agent",
  "infectious:security_zombie",
  "infectious:zombie_police",
  "infectious:swat_zombie",
  "infectious:zombie_soldier",
  "infectious:zombie_prisoner",
  "infectious:psycho_zombie",
  "infectious:zombie_bomber",
  "infectious:franklin_zombie",
  "infectious:baseball_bat_zombie",
  "infectious:zombie_archer",
  "infectious:firework_zombie",
  "infectious:zombie_warrior",
  "infectious:zombie_horseman",
  "infectious:burning_zombie",
  "infectious:burned_zombie",
  "infectious:withered_zombie",
  "infectious:frozen_zombie",
  "infectious:muddy_zombie",
  "infectious:balloon_zombie_no_balloon",
  "infectious:zombie_horseman_no_horse",
  "infectious:zombie_miner_spawn",
  "infectious:zombie_screamer",
  "infectious:tower_zombie_1",
  "infectious:tower_zombie_2",
  "infectious:tower_zombie_3",
  "infectious:tower_zombie_4",
  "infectious:zombified_bird",
  "undead_revamp2:theheavy",
  "undead_revamp2:bomber",
  "undead_revamp2:thebidy",
  "undead_revamp2:thepregnant",
  "undead_revamp2:theimmortal",
  "undead_revamp2:invisiimmortal",
  "undead_revamp2:thedungeon",
  "undead_revamp2:thesmoker",
  "undead_revamp2:theswarmer",
  "undead_revamp2:thegliter",
  "undead_revamp2:the_moonflower",
  "undead_revamp2:sucker",
  "undead_revamp2:thehunter",
  "undead_revamp2:thesomnolence",
  "undead_revamp2:bigsucker",
  "undead_revamp2:invisiblebidy",
  "undead_revamp2:thelurker",
  "undead_revamp2:theposessive",
  "undead_revamp2:therabidus",
  "zombie_extreme:infected",
  "zombie_extreme:runner",
  "zombie_extreme:infected_police",
  "zombie_extreme:infected_hazmat",
  "zombie_extreme:infected_military",
  "zombie_extreme:infected_juggernaut",
  "zombie_extreme:boomer",
  "zombie_extreme:crawler",
  "zombie_extreme:demolisher",
  "zombie_extreme:ram",
  "zombie_extreme:chainsaw",
  "zombie_extreme:explosive_infected",
  "zombie_extreme:night_hunter",
  "zombie_extreme:rat_king",
  "zombie_extreme:clicker",
  "zombie_extreme:inflated",
  "zombie_extreme:faceless",
  "zombie_extreme:goon",
  "zombie_extreme:revived",
  "zombie_extreme:divided",
  "zombie_extreme:devestated",
  "zombie_extreme:parasite",
  "zombie_extreme:zero_patient",
  "zombie_extreme:fetus",
  "mutantmonsters:mutant_skeleton",
  "mutantmonsters:mutant_zombie",
  "mutantmore:mutant_wither_skeleton",
  "mutantmore:mutant_husk",
  "mutantmore:mutant_frozen_zombie",
  "mutantmore:mutant_jungle_zombie",
  "specialmobs:brutezombie",
  "specialmobs:firezombie",
  "specialmobs:frozenzombie",
  "specialmobs:giantzombie",
  "specialmobs:hungryzombie",
  "specialmobs:madscientistzombie",
  "specialmobs:plaguezombie",
  "specialmobs:abyssaldrowned",
  "specialmobs:brutedrowned",
  "specialmobs:frozendrowned",
  "specialmobs:giantdrowned",
  "specialmobs:hungrydrowned",
  "specialmobs:knightdrowned",
  "specialmobs:plaguedrowned",
  "specialmobs:zombifiedpiglin",
  "specialmobs:brutezombifiedpiglin",
  "specialmobs:fishingzombifiedpiglin",
  "specialmobs:giantzombifiedpiglin",
  "specialmobs:hungryzombifiedpiglin",
  "specialmobs:knightzombifiedpiglin",
  "specialmobs:plaguezombifiedpiglin",
  "specialmobs:plaguezombifiedpiglin",
  "specialmobs:bruteskeleton",
  "specialmobs:fireskeleton",
  "specialmobs:gatlingskeleton",
  "specialmobs:giantskeleton",
  "specialmobs:knightskeleton",
  "specialmobs:ninjaskeleton",
  "specialmobs:poisonskeleton",
  "specialmobs:sniperskeleton",
  "specialmobs:spitfireskeleton",
  "specialmobs:weatheredskeleton",
  "specialmobs:brutewitherskeleton",
  "specialmobs:gatlingwitherskeleton",
  "specialmobs:giantwitherskeleton",
  "specialmobs:knightwitherskeleton",
  "specialmobs:ninjawitherskeleton",
  "specialmobs:sniperwitherskeleton",
  "specialmobs:spitfirewitherskeleton",
  "specialmobs:babyghast",
  "specialmobs:corporealshiftghast",
  "specialmobs:fighterghast",
  "specialmobs:kingghast",
  "specialmobs:queenghast",
  "specialmobs:slabghast",
  "specialmobs:unholyghast",
  "mozombies_wave:disco_zombie",
  "mozombies_wave:zombie_chef",
  "mozombies_wave:zombie_cyborg",
  "mozombies_wave:zombie_king",
  "mozombies_wave:zombie_knight",
  "mozombies_wave:zombie_miner",
  "mozombies_wave:zombie_pa",
  "mozombies_wave:zombie_pirate",
  "mozombies_wave:zombie_dwarf",
  "mozombies_wave:nether_zombie",
  "mozombies_wave:zombie_creeper",
  "graveyard:reaper",
  "graveyard:acolyte",
  "graveyard:corrupted_vindicator",
  "graveyard:corrupted_pillager",
  "graveyard:ghoul",
  "graveyard:revenant",
  "graveyard:wraith",
  "graveyard:ghouling",
  "graveyard:nameless_hanged",
  "tinyskeletons:baby_skeleton",
  "tinyskeletons:baby_wither_skeleton",
  "tinyskeletons:baby_stray",
  "giants_overhaul:giant_drowned",
  "giants_overhaul:giant_husk",
  "rotted:marine",
  "rotted:doctor",
  "rotted:farmer",
  "rotted:shielder",
  "rotted:miner",
  "rotted:adventurer",
  "rotted:flusk",
  "rottencreatures:burned",
  "rottencreatures:frostbitten",
  "rottencreatures:swampy",
  "rottencreatures:undead_miner",
  "rottencreatures:mummy",
  "rottencreatures:glacial_hunter",
  "rottencreatures:hunter_wolf",
  "rottencreatures:dead_beard",
  "rottencreatures:zombie_lackey",
  "rottencreatures:skeleton_lackey",
  "rottencreatures:immortal",
  "rottencreatures:zap",
  "undeadvariants:zombified_piglin_brute",
  "undeadvariants:husk_villager",
  "undeadvariants:drowned_piglin",
  "undeadvariants:drowning_piglin",
  "undeadvariants:drowned_villager",
  "undeadvariants:drowning_zombie_villager"
]